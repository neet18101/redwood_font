...
function page() {
    ...
    console.log('pathname:', pathname); // Check router status
    const pathSegments = pathname.split('/').filter(segment => segment); // Split and filter empty segments
    // let catIndx  = pathSegments?.length > 2 ? pathSegments?.length - 2 : 0;
    let catIndx  = 0;
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(`/api/forntend-api/get-menu-by-slug?slug=${pathname}`);
                            ...
                            <article className="portfolio-single">
                                {Array.isArray(catData) && catData.map((item) => (
                                    <>
                                        ...
                                        <div className="pbmit-short-description" key={item.id}>
                                            <h3>Aluminum Windows</h3>
                                            <p className="text-start" dangerouslySetInnerHTML={{ __html: item?.content }}></p>
                                        </div>
                                                ...
                                                <div className="row">
                                                    <div className="col-md-12">
                                                        <div className="d-flex image-container">  
                                                            {Array.isArray(item?.service_slider) && item?.service_slider.map((imgUrl) => (
